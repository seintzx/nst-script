...
(function () {
	...
	childList:true,subtree:true});}function Vt(t){const n=async()=>{
	...
	}).observe(t,{childList:true,subtree:true});}(t);}const Ln=[{key:"ezdepositcompany",
	name:"fast deposit",
	description:"deposit money with a double click (double-click on dollar sign)",
	onpage:V.Company,category:Z.Company,enabled:false,readyStates:["interactive"],
	run:()=>nt(xn,Q.ezmoneydeposit)},{key:"ezdepositfaction",name:"fast deposit",
	description:"deposit money with a double click (double-click on dollar sign)",
	onpage:V.Faction,category:Z.Faction,enabled:false,readyStates:["interactive"],
	run:()=>nt(xn,Q.ezmoneydeposit)},{key:"ezdepositproperty",name:"fast deposit",
	description:"deposit money with a double click (double-click on dollar sign)",
	onpage:V.Properties,category:Z.Properties,enabled:false,
	readyStates:["interactive"],run:()=>nt(xn,Q.ezmoneydeposit)
	}],{reloadone:Sn,reloadtwo:_n,weapon:Tn}=f(),In=ft("user2ID")??"",Mn={primary:1,
	...
	end:()=>({user2ID:In}),finish:()=>{var t;return {step:"finish",
	fightResult:null==(t=f())?void 0:t.outcome}},check:()=>({user2ID:In}),
	start:()=>({step:"startFight",user2ID:In}),hit:()=>({step:"attack",user2ID:In,
	user1EquipedItemID:zn()})};function qn(t){p({outcome:t.target.id});}
	function An(){var t
	...
	;if(x[t]())return Mn[t];Pn++;}while(Pn<3);return 999}async function Nn(t){
	const n=t.target;n.disabled=true;const e=An(),o=$n[e]()
	...
	console.debug(t),Dn=t,On=t.DB;const e=An();n.innerText=jn[e](),n.disabled=!1;
	}(await t.clone().json(),n);}catch(a){console.error(a);}}function Hn(t){
	...
	classList:"nst-outcome",onchange:qn}),e=j({classList:"nst-outcome"})
	...
	;o.appendChild(i),n.appendChild(e),n.appendChild(o);}(t);}const Cn=[{
	...
	run:()=>nt(Hn,Q.fighterino),condition:()=>"attack"===pt()}],Fn={Argentina:167,
	...
	innerText:n.desc.join()});e.selected=r===t,i.appendChild(e);}),i}(t,ne),i=N({
	...
	const r=(e.faction_perks??[]).filter(n=>n.includes(t+" gym gains")),a=(o??[]).filter(n=>n.includes(t+" gym gains")),s=ae(r,/\d+/),c=ae(i,/\d+/),l=ae(a,/\d+/),u=j({
	...
	}).observe(t,{childList:true,subtree:true});}const qe=[{key:"pointsnoconfirm",
	name:"points quick buy",description:"remove confirmation in points market",
	onpage:V.Points,category:Z.Points,enabled:false,readyStates:["interactive"],
	run:()=>nt($e,Q.pointsnoconfirm)}],Ae=Q.pokerNameSelector;function ze(t){var n
	...
	t.querySelectorAll(Ae).forEach(ze),new MutationObserver(t=>{var n
	;for(const e of t)for(const t of Array.from(e.addedNodes))t instanceof HTMLElement&&(t.matches(Ae)&&ze(t),
	null==(n=t.querySelectorAll)||n.call(t,Ae).forEach(ze));}).observe(t,{
	childList:true,subtree:true});}const He=[{key:"pokername",name:"poker name link",
	...
	condition:()=>"holdem"===pt()}];function Ce(t,n){var e=Object.keys(t)
	...
	;n%2?Ce(Object(e),true).forEach(function(n){Be(t,n,e[n]);
	}):Object.getOwnPropertyDescriptors?Object.defineProperties(t,Object.getOwnPropertyDescriptors(e)):Ce(Object(e)).forEach(function(n){
	...
	var So="Sortable"+(new Date).getTime(),_o=[],To={initializeByDefault:true},Io={
	...
	n.indexOf(e)>=0||{}.propertyIsEnumerable.call(t,e)&&(i[e]=t[e]);}return i}(e,Mo)
	;Io.pluginEvent.bind(ki)(t,n,Fe({dragEl:Eo,parentEl:Po,ghostEl:jo,rootEl:$o,
	nextEl:qo,lastDownEl:Ao,cloneEl:zo,cloneHidden:No,dragStarted:Qo,putSortable:Uo,
	activeSortable:ki.active,originalEvent:o,oldIndex:Ho,oldDraggableIndex:Fo,
	newIndex:Co,newDraggableIndex:Ro,hideGhostForTarget:gi,unhideGhostForTarget:bi,
	...
	;var b=Fe(Fe({},m),Io.getEventProperties(o,n));for(var y in b)h[y]=b[y]
	;e&&e.dispatchEvent(h),v[g]&&v[g].call(n,h);}}(Fe({putSortable:Uo,cloneEl:zo,
	targetEl:Eo,rootEl:$o,oldIndex:Ho,oldDraggableIndex:Fo,newIndex:Co,
	newDraggableIndex:Ro},t));}
	var Eo,Po,jo,$o,qo,Ao,zo,No,Ho,Co,Fo,Ro,Bo,Uo,Go,Wo,Xo,Jo,Ko,Yo,Qo,Vo,Zo,ti,ni,ei=false,oi=false,ii=[],ri=false,ai=false,si=[],ci=false,li=[],ui="undefined"!=typeof document,di=Ye,fi=Xe||We?"cssFloat":"float",pi=ui&&!Qe&&!Ye&&"draggable"in document.createElement("div"),mi=function(){
	...
	;for(var r in Io.initializePlugins(this,t,i),i)!(r in n)&&(n[r]=i[r])
	;for(var a in vi(n),
	...
	;return o.toString(36)}function Ti(t){return setTimeout(t,0)}function Ii(t){
	...
	;o.checked&&li.push(o);}
	}(e),!Eo&&!(/mousedown|pointerdown/.test(r)&&0!==t.button||o.disabled)&&!c.isContentEditable&&(this.nativeDraggable||!Ke||!s||"SELECT"!==s.tagName.toUpperCase())&&!((s=oo(s,o.draggable,e,false))&&s.animated||Ao===s)){
	if(Ho=vo(s),Fo=vo(s,o.draggable),"function"==typeof l){
	...
	var c=fo(e)
	;if($o=r,Po=(Eo=e).parentNode,qo=Eo.nextSibling,Ao=e,Bo=a.group,ki.dragged=Eo,
	...
	D("revert"),ki.eventCanceled||(qo?$o.insertBefore(Eo,qo):$o.appendChild(Eo)),
	...
	Zo=g;var T=a.nextElementSibling,I=false,M=xi($o,r,Eo,n,a,e,t,I=1===g)
	;if(false!==M)return 1!==M&&-1!==M||(I=1===M),
	ci=true,setTimeout(Si,30),O(),I&&!T?r.appendChild(Eo):a.parentNode.insertBefore(Eo,I?T:a),
	...
	function P(){Co=vo(Eo),Ro=vo(Eo,s.draggable),Oo({sortable:p,name:"change",
	toEl:r,newIndex:Co,newDraggableIndex:Ro,originalEvent:t});}},
	...
	;Co=vo(Eo),Ro=vo(Eo,e.draggable),Do("drop",this,{evt:t
	}),Po=Eo&&Eo.parentNode,Co=vo(Eo),
	Ro=vo(Eo,e.draggable),ki.eventCanceled||(ei=false,
	ai=false,ri=false,clearInterval(this._loopId),
	clearTimeout(this._dragStartTimer),Ii(this.cloneId),
	Ii(this._dragStartId),this.nativeDraggable&&(to(document,"drop",this),
	...
	newIndex:null,newDraggableIndex:null,originalEvent:t}),$o!==Po?(Co>=0&&(Oo({
	...
	originalEvent:t})),Uo&&Uo.save()):Co!==Ho&&Co>=0&&(Oo({sortable:this,
	name:"update",toEl:Po,originalEvent:t}),Oo({sortable:this,name:"sort",toEl:Po,
	originalEvent:t})),ki.active&&(null!=Co&&-1!==Co||(Co=Ho,Ro=Fo),Oo({
	sortable:this,name:"end",toEl:Po,originalEvent:t
	}),this.save())))),this._nulling();},_nulling:function(){
	Do("nulling",this),$o=Eo=Po=jo=qo=zo=Ao=No=Go=Wo=Qo=Co=Ro=Ho=Fo=Vo=Zo=Uo=Bo=ki.dragged=ki.ghost=ki.clone=ki.active=null,
	...
	;var o=Io.modifyOption(this,t,n);e[t]=void 0!==o?o:n,"group"===t&&vi(e);},
	...
	;Eo.parentNode!=$o||this.options.group.revertClone?qo?$o.insertBefore(zo,qo):$o.appendChild(zo):$o.insertBefore(zo,Eo),
	...
	closest:oo,toggleClass:ao,clone:xo,index:vo,nextTick:Ti,cancelNextTick:Ii,
	...
	;t.utils&&(ki.utils=Fe(Fe({},ki.utils),t.utils)),Io.mount(t);});
	},ki.create=function(t,n){return new ki(t,n)},ki.version="1.15.6"
	;var Mi,Di,Oi,Ei,Pi,ji,$i=[],qi=false;function Ai(){$i.forEach(function(t){
	...
	;Di!==e&&(Di=e,Ai(),Mi=n.scroll,i=n.scrollFn,true===Mi&&(Mi=bo(e,true)))
	;var d=0,f=Mi;do{
	var p=f,m=fo(p),h=m.top,v=m.bottom,g=m.left,b=m.right,y=m.width,w=m.height,k=void 0,x=void 0,L=p.scrollWidth,S=p.scrollHeight,_=so(p),T=p.scrollLeft,I=p.scrollTop
	...
	;$i[d].vx==M&&$i[d].vy==D&&$i[d].el===p||($i[d].el=p,
	$i[d].vx=M,$i[d].vy=D,clearInterval($i[d].pid),
	0==M&&0==D||(u=true,$i[d].pid=setInterval(function(){
	...
	}.bind({layer:d}),24))),d++;}while(n.bubbleScroll&&f!==l&&(f=bo(f,false)));qi=u;}
	},30),Hi=function(t){
	...
	;function Ci(){}function Fi(){}Ci.prototype={startIndex:null,
	...
	this.sortable.animateAll(),e&&e.animateAll();},drop:Hi},Ue(Ci,{
	...
	e.animateAll();},drop:Hi},Ue(Fi,{pluginName:"removeOnSpill"
	...
	zi(),Ai(),clearTimeout(io),io=void 0;},nulling:function(){
	Pi=Di=Mi=qi=ji=Oi=Ei=null,$i.length=0;},_handleFallbackAutoScroll:function(t){
	...
	;!qi||ji&&o===Oi&&i===Ei||(ji&&zi(),ji=setInterval(function(){
	var r=bo(document.elementFromPoint(o,i),true)
	;r!==a&&(a=r,Ai()),Ni(t,e.options,r,n);},10),Oi=o,Ei=i);}else {
	if(!this.options.bubbleScroll||bo(r,true)===uo())return void Ai()
	;Ni(t,this.options,bo(r,false),false);}}},Ue(t,{pluginName:"scroll",
	initializeByDefault:true})}),ki.mount(Fi,Ci);const Ri={box:t=>{var n;const e=G({
	...
	;s&&(s.innerHTML="",Zi.includes(a)?await async function(t,n){var e
	...
	"isEquipped"in t&&(t.isEquipped=false);}),l[o].isEquipped=a,p({[w]:l}),rr());
	...
	classList:"nst-box-button",onclick:or}),u={src:s,alt:o,classList:"nst-box-img"
	...
	async function Mr(t){const n=await t.clone().json()
	;if("lobby"!==(null==n?void 0:n.step))return t;const e=n.data
	;for(const o of Object.values(e)){const{ID:t,timeCreated:n}=o
	;void 0!==t&&(_r[t]=n);}
	return null!==Tr&&clearInterval(Tr),Tr=setInterval(Ir,1e3),t}const Dr=[{
	...
	run:()=>on(Mr,"russianRouletteData"),condition:()=>"russianRoulette"===pt()}]
	...
	run:()=>nt(jr,Q.auctionrwbonus)}];function qr(t){
	...
	})),e.appendChild(O("br",{})));}function Ar(t){const n=/(itemsContainner|row)/
	;new MutationObserver(t=>{
	...
	;if(e.length)for(const t of Array.from(e))qr(t);}}).observe(t,{childList:true,
	...
	run:()=>nt(Ar,Q.rwbonusbazaar)}];function Nr(t){
	...
	})),t.insertAdjacentElement("afterend",O("br",{}));}function Hr(t){
	const n=t.querySelectorAll(".bonus-attachment-icons");n.length&&n.forEach(Nr);}
	const Cr=[{key:"rwbonusdisplay",name:"rw bonus display",
	description:"display rw weapon bonus over the item instead of tooltip",
	onpage:V.DisplayCase,category:Z.DisplayCase,enabled:false,
	readyStates:["interactive"],run:()=>nt(Hr,Q.rwbonusdisplay)}];function Fr(t){
	...
	},Ia=/^(\d+)x (.+)$/;function Ma(t,n){const e=t.match(Ia);if(!e)return null
	...
	let e=0,o=0,i=0;for(const r of t){const t=Ma(r,n);t&&(e+=t.qty,o+=t.value*t.qty,
	...
	const Aa=[...et,...at,...It,...Dt,...Pt,...qt,...Nt,...Ft,...rn,...cn,...un,...pn,...hn,...Ln,...Cn,...Vn,...re,...ce,...le,...de,...fe,...me,...he,...ge,...ye,...ke,...Ie,...Me,...Ee,...qe,...He,...Ji,...Vi,...cr,...hr,...br,...wr,...Sr,...Dr,...$r,...zr,...Cr,...Ur,...Jr,...da,...ka,...La,..._a,...Oa,{
	...
	;t!==ja&&(ja=t,/disposal/.test(mt())&&(nt(qa,Q.disposalhighlight),
	clearInterval($a),$a=null));},500));},condition:()=>/crimes/.test(pt())}]
	;function za(){var t,n;console.debug("nst - init default start");const e={}
	;for(const r of Aa)e[r.key]={name:r.name,description:r.description,
	...
	}function Na(t,n,e){return q({id:"nst-"+t,type:"password",
	classList:"nst-input m-2",maxLength:n,required:true,value:e})}function Ha(t,n=false){
	const e=document.getElementById("nst-ts-message");var o
	;e&&((o=e).innerText="",o.classList.remove("text-nstgreen"),
	e.innerText=t,e.classList.toggle("text-nstgreen",n));}async function Ca(){
	...
	}async function Fa(){const t=document.getElementById("nst-apikey");if(!t)return
	...
	;p({userdata:e}),cs(),Ha("Key Saved",!0);}catch(o){Ha(o||"Unknown error");}}
	async function Ra(){
	confirm("Are you sure you want to reset?")&&(m("nstdata"),window.location.reload());
	}function Ba(t){const n=document.getElementById(t);if(!n)return;n.innerHTML=""
	...
	})),a="tsapikey",s=Na(a,19,(f(a)??"")+""),c=P({id:"nst-tslogin",
	classList:"torn-btn m-2",innerText:"save",onclick:Ca})
	;i.appendChild(o),i.appendChild(r),
	i.appendChild(s),i.appendChild(c),n.appendChild(i);}function Ua(t){
	...
	classList:"nst-message"})),c="apikey",l=Na(c,16,(f(c)??"")+""),u=P({
	id:"nst-login",classList:"torn-btn m-2",innerText:"save",onclick:Fa}),d=P({
	id:"nst-reset",classList:"torn-btn m-2",innerText:"reset",onclick:Ra})
	;a.appendChild(r),
	a.appendChild(s),a.appendChild(l),a.appendChild(u),a.appendChild(d),
	n.appendChild(a);}function Ga(t,...n){const e=j({classList:"nst-block"
	}),o=O("h4",{classList:"nst-block-title",innerText:t})
	;return e.appendChild(o),n.forEach(t=>e.appendChild(t)),e}function Wa(t,n,e){
	const o=j({id:n,classList:"nst-outcome"});o.addEventListener("change",t=>{
	const n=t.target;n&&"radio"===n.type&&e(n.id);})
	;const i=document.createDocumentFragment()
	;return t.forEach(t=>i.appendChild(U(t,n))),o.appendChild(i),o}function Xa(t){
	const n=document.getElementById(t);if(!n)return;n.innerHTML=""
	...
	}()),e.appendChild(Ga("default outcome",Wa(["leave","mug","hospitalize"],"outcome",t=>p({
	...
	}}),Ga("weapon order",n,B({id:"nst-temp"},"use temp first"),B({id:"nst-assaperk"
	...
	},"secondary")),t.appendChild(n),Ga("weapon reload",t)}()),n.appendChild(e);}
	function Ja(t){const n=document.getElementById(t);if(!n)return;n.innerHTML=""
	;const e=document.createDocumentFragment()
	;e.appendChild(Ga("default outcome",Wa(["easy-leave","easy-mug","easy-hospitalize"],"easy-outcome",t=>p({
	easyoutcome:t.replace(/easy-/g,"")
	})))),e.appendChild(Ga("default weapon",Wa(["easy-primary","easy-secondary","easy-melee"],"easy-weapon",t=>p({
	easyweapon:t.replace(/easy-/g,"")})),B({id:"nst-easy-temp"},"use temp first"))),
	n.appendChild(e);}let Ka=null;function Ya(t){const n=document.getElementById(t)
	;return n?(n.innerHTML="",n):null}function Qa(t,n,e){
	t.className=e,t.innerHTML=`<b>${n}</b>`,t.classList.remove("hidden");}
	function Va(t){const n=Ya(t);if(!n)return;const e=j({id:"nst-export-wrap",
	...
	;Ka&&window.URL.revokeObjectURL(Ka),Ka=window.URL.createObjectURL(new Blob([n],{
	type:"application/json"
	})),t.setAttribute("href",Ka+"#/nst-export"),t.setAttribute("download","nst-export.json"),
	t.classList.remove("hidden");}(o)});e.append(i,o),n.appendChild(e);}
	function Za(t){const n=Ya(t);if(!n)return;const e=H({})
	...
	za(),Qa(n,"Import Successful","text-nstgreen");}catch{
	Qa(n,"Import Unsuccessful","text-nstred");}},o.onerror=t=>{
	Qa(n,"Import Unsuccessful","text-nstred"),console.error(t);},o.readAsText(e[0]);
	}(i,e)});i.classList.add("hidden");const r=P({id:"nst-import-btn",
	innerText:"import",classList:"torn-btn m-2",onclick:()=>i.click()})
	;o.append(r,i,e),n.appendChild(o);}const ts={"Data Storage":"Only locally",
	"Data Sharing":"Nobody",
	"Purpose of Use":"Optimization / Maintenance but only end user has access",
	"Key Storage & Sharing":"Stored locally but not shared",
	"Key Access Level":"Limited or Custom (see below)"};function ns(t,n){const e=j({
	...
	}function es(t){const n=document.getElementById(t);if(!n)return;n.innerHTML=""
	;const e=document.createDocumentFragment()
	;e.appendChild(ns("Terms of Service",ts)),
	e.appendChild(ns("Api usage",_)),n.appendChild(e);}
	const os=["spring","summer","autumn","winter"];function is(){
	const t=os.map(t=>document.querySelector(".content-wrapper."+t)).find(Boolean)
	...
	rs(a,"tos",es),rs(a,"api",Ua),function(t){
	...
	null==e||e.classList.toggle("hidden");}(r),cs();}(t);}function rs(t,n,e=()=>{}){
	...
	classList:"nst-menu-page",innerText:n,onclick:as})
	...
	function ss(t){const n=document.getElementById(t);if(!n)return;n.innerHTML=""
	...
	}),s.appendChild(c),n.appendChild(s);}function cs(){if(!gt())return;za()
	;const t=document.querySelector("#nst-ul-main");if(!t)return;const n={tsapi:Ba,
	attack:Xa,"easy-fight":Ja,export:Va,import:Za}
	;Object.keys(n).forEach(e=>rs(t,e,n[e]))
	;const e=f("script"),o=Object.keys(e).sort().reduce((t,n)=>(t[n]=null==e?void 0:e[n],
	t),{})
	;Array.from(new Set(Object.values(o).map(t=>t.category).filter(Boolean))).sort().forEach(n=>rs(t,n,ss));
	}function ls(t){if(document.getElementById("nst-icon"))return
	;const n=t.querySelector('ul[class^="status-icon"]');if(!n)return;const e=z({
	id:"nst-icon",classList:"nst-icon"})
	;n.insertBefore(e,n.firstChild),e.addEventListener("click",is);}
	const us=window.location.pathname.replace(/^\//,"").replace(/\.php$/,""),ds={
	loading:()=>{
	nt(D,"head"),nt(ls,"#sidebar"),gt()&&(document.onreadystatechange||(document.onreadystatechange=()=>{
	const t=document.readyState;ds[t]&&ds[t]();}),st(us,"loading"),kt());},
	interactive:()=>{gt()&&st(us,"interactive");},complete:()=>{st(us,"complete");}}
	;ds[document.readyState]&&ds[document.readyState]();}();
